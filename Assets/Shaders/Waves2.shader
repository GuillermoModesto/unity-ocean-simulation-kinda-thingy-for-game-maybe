Shader "Custom/Water_SimpleZWrite"
{
    Properties
    {
        ...
        _SandColor      ("Sand / Shore", Color)      = (0.90, 0.92, 0.78, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Piecewise smooth tropical ramp (drop-in replacement)
            float3 TropicalRamp(float h, float s)
            {
                float k0=0.00, k1=0.12, k2=0.28, k3=0.45, k4=0.62, k5=0.78, k6=0.92, k7=1.00;
                float w = s * 0.06;
                float3 c;
                ...
                {
                    // Sand stop: desaturate/brighten and make it optional
                    ...
                    // First blend shallow→sand by tSand, then apply global SandStrength
            ...
            {
                // --- height remap to 0..1 with optional bias ---
                ...
                // Crest foam (slope + tiny curvature + animated noise; biased to peaks)
